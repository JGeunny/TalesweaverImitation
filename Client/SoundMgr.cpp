...
void CSoundMgr::MyPlaySound(TCHAR * pSoundKey, CHANNELID eID)
{
	for (int i = 0; i < CHANNELID::MAXCHANNEL; ++i)
	{
		FMOD_Channel_SetVolume(m_pChannelArr[i], 0.5f);
	}

	FMOD_Channel_SetVolume(m_pChannelArr[CHANNELID::HIT], 0.2f);
	FMOD_Channel_SetVolume(m_pChannelArr[CHANNELID::EXPLO], 1.f);
